...
public class DoubleDes
{
    ...
    public final static long MASK_32_BITS    = 0xFFFFFFFFL;
    protected final static int  MASK_28_BITS    = 0x0FFFFFFF;
    public final static int  NUM_OF_ROUNDS   = 16;
    ...
    private final RoundKeyGenerator keygen1;
    private final RoundKeyGenerator key2;
    private final FeistelFunction feistel;

    public DoubleDes()
    {

        keygen1 = new RoundKeyGenerator();
        key2 = new RoundKeyGenerator();
    ...
    {
        long[] roundKeys = keygen1.generateRoundKeys(key);
        ...
        for (int i = 0; i < DoubleDes.NUM_OF_ROUNDS; i++)
        {
            if (encrypt)
                ...
                FOutput = feistel.F(rightHalf, roundKeys[(DoubleDes.NUM_OF_ROUNDS-1)-i]);
    ...
    {
        return DoubleDes.genericPermutation(input, IP, 64);
    }


    private long finalPermutation(long input)
    {
        return DoubleDes.genericPermutation(input, FP, 64);
    }
    ...
    //second encryption
    private long cipher2(long block, long key, boolean encrypt)
    {
        long[] roundKeys = key2.generateRoundKeys(key);
        block = initialPermutation(cipher(block, key, encrypt));
